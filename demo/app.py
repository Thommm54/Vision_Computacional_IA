...
def detectar_baches(imagen: np.ndarray, confianza: float):
    ...
    results = model.predict(source=img_bgr, conf=confianza, verbose=False)[0]
    boxes  = results.boxes
    ...
    if n_det > 0:
        for box in boxes:
            ...
            color = (0, 0, intensity)  # Rojo en BGR
            ...
            cv2.putText(img_out, label, (x1 + 3, y1 - 5),
...
with gr.Blocks(
    ...
    with gr.Row(equal_height=True):
        # Logo izquierdo (opcional — si no tienes logo.png, comenta estas líneas)
        if os.path.exists("logo.png"):
            with gr.Column(scale=1, min_width=100):
                gr.Image(value="logo.png", show_label=False,
                         interactive=False, container=False, height=90)
        else:
            with gr.Column(scale=1, min_width=100):
                gr.Markdown("")
        ...
        with gr.Column(scale=6):
            ...
            gr.Markdown(
                "Modelo **YOLO11n** entrenado sobre *Pothole Detection Dataset* · Fase 2 — Deep Learning",
                elem_id="subtitulo",
            )

        # Logo derecho (opcional)
        if os.path.exists("logo_1.png"):
            with gr.Column(scale=1, min_width=100):
                gr.Image(value="logo_1.png", show_label=False,
                         interactive=False, container=False, height=90)
        else:
            with gr.Column(scale=1, min_width=100):
                gr.Markdown("")
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            slider_conf = gr.Slider(
                ...
                info="0.4 = balance recall/precisión (valor usado en entrenamiento)",
            )
